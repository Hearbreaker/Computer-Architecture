...
L1Cache_Controller::bs_issuePUTS(L1Cache_TBE*& m_tbe_ptr, L1Cache_Entry*& m_cache_entry_ptr, const Address& addr)
{
    ...
    if (!((m_cache_entry_ptr != NULL))) {
        char c;
        cerr << "Runtime Error at MSI-cache.sm:386, Ruby Time: "
...
L1Cache_Controller::bm_issuePUTM(L1Cache_TBE*& m_tbe_ptr, L1Cache_Entry*& m_cache_entry_ptr, const Address& addr)
{
    ...
    if (!((m_cache_entry_ptr != NULL))) {
        char c;
        cerr << "Runtime Error at MSI-cache.sm:399, Ruby Time: "
             << g_eventQueue_ptr->getTime() << ": "
             << "assert failure"
             << ", PID: " << getpid() << endl
             << "press return to continue." << endl;
    ...
    assert(in_msg_ptr != NULL); // Check the cast result
{
    ...
    if (!((m_cache_entry_ptr != NULL))) {
        char c;
        cerr << "Runtime Error at MSI-cache.sm:412, Ruby Time: "
             << g_eventQueue_ptr->getTime() << ": "
             << "assert failure"
             << ", PID: " << getpid() << endl
             << "press return to continue." << endl;
    ...
    assert(in_msg_ptr != NULL); // Check the cast result
{
    ...
    if (!((m_tbe_ptr != NULL))) {
        char c;
        cerr << "Runtime Error at MSI-cache.sm:426, Ruby Time: "
             << g_eventQueue_ptr->getTime() << ": "
             << "assert failure"
             << ", PID: " << getpid() << endl
             << "press return to continue." << endl;
...
L1Cache_Controller::de_sendDataFromCacheToDir(L1Cache_TBE*& m_tbe_ptr, L1Cache_Entry*& m_cache_entry_ptr, const Address& addr)
{
    ...
    if (!((m_cache_entry_ptr != NULL))) {
        char c;
        cerr << "Runtime Error at MSI-cache.sm:439, Ruby Time: "
             << g_eventQueue_ptr->getTime() << ": "
             << "assert failure"
             << ", PID: " << getpid() << endl
             << "press return to continue." << endl;
...
L1Cache_Controller::dee_sendDataFromTBEToDir(L1Cache_TBE*& m_tbe_ptr, L1Cache_Entry*& m_cache_entry_ptr, const Address& addr)
{
    ...
    if (!((m_tbe_ptr != NULL))) {
        char c;
        cerr << "Runtime Error at MSI-cache.sm:451, Ruby Time: "
             << g_eventQueue_ptr->getTime() << ": "
             << "assert failure"
             << ", PID: " << getpid() << endl
             << "press return to continue." << endl;
    ...
    DPRINTF(RubyGenerated, "executing i_allocateL1CacheBlock\n");
        if ((m_cache_entry_ptr != NULL)) {
            if (((getAccessPermission(addr)) != AccessPermission_Invalid)) {
                char c;
                cerr << "Runtime Error at MSI-cache.sm:478, Ruby Time: "
                     << g_eventQueue_ptr->getTime() << ": "
...
if (!((m_cache_entry_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:513, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
#endif
;
DPRINTF(RubySlicc, "MSI-cache.sm:514: %s\n", (*m_cache_entry_ptr).m_DataBlk);
...
if (!((m_cache_entry_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:524, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
#endif
;
DPRINTF(RubySlicc, "MSI-cache.sm:525: %s\n", (*m_cache_entry_ptr).m_DataBlk);
...
if (!((m_cache_entry_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:534, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
#endif
;
DPRINTF(RubySlicc, "MSI-cache.sm:535: %s\n", (*m_cache_entry_ptr).m_DataBlk);
...
if (!((m_cache_entry_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:545, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
#endif
;
DPRINTF(RubySlicc, "MSI-cache.sm:546: %s\n", (*m_cache_entry_ptr).m_DataBlk);
...
if (!((m_cache_entry_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:558, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
if (!((m_tbe_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:569, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
L1Cache_Controller::forward_eviction_to_cpu(L1Cache_TBE*& m_tbe_ptr, L1Cache_Entry*& m_cache_entry_ptr, const Address& addr)
{
    ...
    DPRINTF(RubySlicc, "MSI-cache.sm:579: forward_eviction to cpu; send_evictions is %s\n", m_send_evictions);
    if (m_send_evictions) {
        DPRINTF(RubySlicc, "MSI-cache.sm:581: Sending invalidation for %s to the CPU\n", addr);
...
if (!((m_cache_entry_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:603, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
...
if (!((m_tbe_ptr != NULL))) {
    char c;
    cerr << "Runtime Error at MSI-cache.sm:604, Ruby Time: "
         << g_eventQueue_ptr->getTime() << ": "
         << "assert failure"
         << ", PID: " << getpid() << endl
         << "press return to continue." << endl;
            ...
            if ((param_type == RubyRequestType_IFETCH)) {
                return L1Cache_Event_Ifetch;
                    ...
                    if (((param_type == RubyRequestType_ST) || (param_type == RubyRequestType_ATOMIC))) {
                        ...
                        char c;
                        cerr << "Runtime Error at MSI-cache.sm:163, Ruby Time: "
                             << g_eventQueue_ptr->getTime() << ": "
