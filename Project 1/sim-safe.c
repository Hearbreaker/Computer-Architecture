...
static counter_t SingleCycle_q1 = 0;	//Single and Double cycle counters
static counter_t DoubleCycle_q1 = 0;	
static counter_t SingleCycle_q2 = 0;	
static counter_t DoubleCycle_q2 = 0;	
...
/* ECE552 Assignment 1 - BEGIN CODE */
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1", "CPI from RAW hazard (q1)", "(sim_num_insn + SingleCycle_q1 + DoubleCycle_q1 * 2) / (sim_num_insn)", NULL); 

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2", "CPI from RAW hazard (q2)", "(sim_num_insn + SingleCycle_q2 + DoubleCycle_q2 * 2) / (sim_num_insn)", NULL);

  stat_reg_counter(sdb, "SingleCycle_q1", "single cycle stalls (q1)", &SingleCycle_q1, SingleCycle_q1, NULL);
			
  stat_reg_counter(sdb, "DoubleCycle_q1", "double cycle stalls (q1)",&DoubleCycle_q1, DoubleCycle_q1, NULL);
			
  stat_reg_counter(sdb, "SingleCycle_q2", "single cycle stalls (q2)",&SingleCycle_q2, SingleCycle_q2, NULL);

  stat_reg_counter(sdb, "DoubleCycle_q2", "double cycle stalls (q2)", &DoubleCycle_q2, DoubleCycle_q2, NULL);
...
stat_reg_formula(sdb, "sim_load_ratio", "load instruction fraction", "sim_num_loads / sim_num_insn", NULL);
...
//Q1**************************************************************************************************************************
{
	int i;
	for (i = 0; i < 3; i++) {
		if (r_in[i] != DNA && reg_ready_q1 [r_in [i]] > sim_num_insn) { //check if any input regs used by output regs.
			if((reg_ready_q1 [r_in [i]] - sim_num_insn) == 1) //Check the cycle difference
				SingleCycle_q1++;
			else if ((reg_ready_q1 [r_in [i]] - sim_num_insn) == 2){
				DoubleCycle_q1++;
				reg_ready_q1 [r_in [i]] = sim_num_insn; //Don't count single cycle hazard if already counted as double cycle.
			}
			else printf("Why is there stuff here?");
...
if(r_out[0] != DNA) //If I1 and I2 has a RAW Hazard. Then I2 need to stall D stage until WB stage of I1.
...
//Q2****************************************************************************************************************************
{
	...
	for (i = 0; i < 3; i++) {
		if (r_in[i] != DNA && reg_ready_q2 [r_in [i]] > sim_num_insn) {
			if ((i == 0) && (MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op)&F_STORE)) {//Corner case 1.
				continue;
			}
			if((reg_ready_q2 [r_in [i]] - sim_num_insn) == 1) //Check the cycle difference
				SingleCycle_q2++;
			else if((reg_ready_q2 [r_in [i]] - sim_num_insn) == 2){
				DoubleCycle_q2++;
				reg_ready_q2 [r_in [i]] = sim_num_insn; //Don't count single cycle hazard if already counted as double cycle.
			}
			else printf("Why is there stuff here?");
...
if ((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)) {
	if (r_out[0] != DNA)
		reg_ready_q2[r_out[0]] = sim_num_insn + 3; //Load finishes at Mem stage.
	if (r_out[1] != DNA)
	...
	if(r_out[0] != DNA)
		reg_ready_q2[r_out[0]] = sim_num_insn + 2; //Finishes at Ex stage. 
	if(r_out[1] != DNA)
