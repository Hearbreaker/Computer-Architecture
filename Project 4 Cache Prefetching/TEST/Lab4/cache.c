	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  if(prefetch_type > 2){	//stride prefetcher
	cp->rpt = (struct rpt *)malloc(sizeof(struct rpt));
	cp->rpt->num_entries = prefetch_type;
	cp->rpt->index_length = log_base2(prefetch_type);
	cp->rpt->table = (struct rpt_entry *)calloc(prefetch_type, sizeof(struct rpt_entry));
  }
  
  if(prefetch_type == 2){	//open-ended prefetcher
	...
	cp->rpt = (struct rpt *)malloc(sizeof(struct rpt));
	cp->rpt->num_entries = 16;
	cp->rpt->index_length = log_base2(16);
	cp->rpt->table = (struct rpt_entry *)calloc(16, sizeof(struct rpt_entry));
...
/* ECE552 Assignment 4 - BEGIN CODE*/
void prefetch_address(struct cache_t *cp, md_addr_t addr){	// prefetch based on the address
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	prefetch_address(cp, addr + cp->bsize);	//simply prefetch the next block
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	//use a simple stride prefetcher
	int index = (get_PC() >> log_base2(sizeof(md_inst_t))) & (cp->rpt->num_entries -1);
	md_addr_t tag = get_PC() >> (cp->rpt->index_length + log_base2(sizeof(md_inst_t)));
	//scenario 1
	if(tag != cp->rpt->table[index].tag){
		//simply set the value and return
		cp->rpt->table[index].tag = tag;
		cp->rpt->table[index].prev_addr = addr;
		cp->rpt->table[index].stride = 0;
		cp->rpt->table[index].state = initial;
	...
	//scenario 2
		...
		enum rpt_state state = cp->rpt->table[index].state;
		bool stride_sign = cp->rpt->table[index].stride_sign;
		bool new_stride_sign = (addr >= prev_addr);
		int flag = (stride_sign == new_stride_sign) && (stride == new_stride);
		enum rpt_state new_state = state_fsm(state, flag);

		cp->rpt->table[index].tag = tag;
		cp->rpt->table[index].prev_addr = addr;
		cp->rpt->table[index].state = new_state;

		if(!flag && (new_state == transient || new_state == no_prediction)){
			//update new stride
			cp->rpt->table[index].stride = new_stride;
			cp->rpt->table[index].stride_sign = new_stride_sign;
		}
		if(new_state == steady){	//only prefetch if in steady state
	...
	if(cache_access_is_miss){	//CZone Prefetcher
		...
		//handle index table
		int index = (addr >> INDEX_OFFSET) & (INDEX_TABLE_SIZE - 1);
		md_addr_t tag = (addr >> (INDEX_OFFSET + log_base2(INDEX_TABLE_SIZE)));
		
		new_entry->linked_ite = &(cp->index_table[index]);	//ghb entry point back to index table entry

		if(cp->index_table[index].linked_entry == NULL){	//empty entry
			cp->index_table[index].linked_entry = new_entry;
			cp->index_table[index].tag = tag;
		}
		else if(cp->index_table[index].tag == tag){		//hit scenario
			new_entry->czone_prev = NULL;
			new_entry->czone_next = cp->index_table[index].linked_entry;
			cp->index_table[index].linked_entry->czone_prev = new_entry;
			cp->index_table[index].linked_entry = new_entry;
			Czone_correlation_prefetch(cp, addr);
		}
		else{	//conflict scenario
			struct ghb_entry *temp = cp->index_table[index].linked_entry;
			struct ghb_entry *temp_next;
			while(temp != NULL){	//free the entire old czone chain
				if(temp == cp->ghb->tail){
		...
		if(cp->ghb->size > MAX_GHB_SIZE){
			delete_ghb_tail(cp);	//GHB is full, delete at tail
		}
	}
	/* ECE552 Assignment 4 - END CODE*/
}
...
/* ECE552 Assignment 4 - BEGIN CODE*/
void delete_ghb_tail(struct cache_t *cp){
	struct ghb_entry *temp = cp->ghb->tail;
	
	cp->ghb->tail = temp->prev;
	
	if(temp->czone_prev == NULL){
		temp->linked_ite->linked_entry = NULL;
		temp->linked_ite->tag = 0;
	}
	
	if(temp->prev != NULL)
		temp->prev->next = temp->next;
	if(temp->czone_prev != NULL)
		temp->czone_prev->czone_next = temp->czone_next;
	
	free(temp);
	cp->ghb->size--;
}

void Czone_correlation_prefetch(struct cache_t *cp, md_addr_t addr){
	int i;
	int prefetch_degree = PREFETCH_DEGREE;
	int correlation_key_reg[2] = {0};
	int correlation_comp_reg[2] = {0};
	
	//identical delta case	
	correlation_key_reg[0] = addr - cp->ghb->head->czone_next->miss_address;	//set correlation key register
	if(cp->ghb->head->czone_next->czone_next != NULL){
		correlation_key_reg[1] = cp->ghb->head->czone_next->miss_address - cp->ghb->head->czone_next->czone_next->miss_address;	//set correlation key register
	}
	else{
		return;	//insufficient history info
	}

	if(correlation_key_reg[0] == correlation_key_reg[1]){
		for(i = 0; i < PREFETCH_DEGREE/2; i++){
			prefetch_address(cp, addr + (i+1) * correlation_key_reg[0]);
		}
		return;
	}
	
	//varying delta case
	int delta_buffer_length = delta_buffer_constructor(cp, addr);	//construct delta buffer
	
	md_addr_t new_addr = addr;
	struct delta_buffer_entry *temp = cp->delta_head;
	
	for(i = 0; i+1 < delta_buffer_length; i++){
		correlation_comp_reg[1] = temp->delta;
		correlation_comp_reg[0] = temp->next->delta;
		if(correlation_comp_reg[1] == correlation_key_reg[1] && correlation_comp_reg[0] == correlation_key_reg[0]){
			if(delta_buffer_length - i < 2 || temp->next->next == NULL){
				delta_buffer_destructor(cp);	//clear the entire deltea buffer
				return;
			}
			else{
				temp = temp->next->next;
				while(prefetch_degree > 0 && temp != NULL){
...
void delta_buffer_destructor(struct cache_t *cp){

	struct delta_buffer_entry *temp;
...
int delta_buffer_constructor(struct cache_t *cp, md_addr_t addr){
	
	int delta;
	int counter = 0;
	struct ghb_entry *temp = cp->ghb->head;
	
	struct delta_buffer_entry *new_delta;
	
	while(temp != NULL){
		if(temp->czone_next != NULL){
			delta = (temp->miss_address) - (temp->czone_next->miss_address);
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	//scenario 2
		...
		bool new_stride_sign = (addr >= prev_addr);
		int flag = (stride_sign == new_stride_sign) && (stride == new_stride);
		enum rpt_state new_state = state_fsm(state, flag);	//determine the new state
...
enum rpt_state state_fsm(enum rpt_state state, int flag){
